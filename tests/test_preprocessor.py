...
# Import the preprocessor module (adjust path as needed)
try:
...
def test_train_test_split_reproducibility(preprocessor, raw_data):
    ...
    pd.testing.assert_frame_equal(X_test1.reset_index(drop=True), X_test2.reset_index(drop=True))
    pd.testing.assert_series_equal(y_train1.reset_index(drop=True), y_train2.reset_index(drop=True))
    pd.testing.assert_series_equal(y_test1.reset_index(drop=True), y_test2.reset_index(drop=True))
...
def test_transform_without_fit_raises(preprocessor, raw_data):
    ...
    """
    X = raw_data.drop(columns=['target'])
    with pytest.raises((RuntimeError, Exception), match="not fitted|fit first|NotFitted"):
